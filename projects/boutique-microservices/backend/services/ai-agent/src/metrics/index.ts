...
export const aiAgentLlmRequestsTotal = new Counter({
  name: 'ai_agent_llm_requests_total',
  help: 'Total number of LLM (chat completion) requests made by the AI agent',
  registers: [register],
});

export const aiAgentLlmDurationSeconds = new Histogram({
  name: 'ai_agent_llm_duration_seconds',
  help: 'Duration of LLM (chat completion) requests in seconds',
  buckets: [0.1, 0.5, 1, 2, 5, 10, 20, 30, 60],
  registers: [register],
});

export const aiAgentToolCallsTotal = new Counter({
  name: 'ai_agent_tool_calls_total',
  help: 'Total number of AI agent tool executions',
  labelNames: ['tool'],
  registers: [register],
});

export const aiAgentToolErrorsTotal = new Counter({
  name: 'ai_agent_tool_errors_total',
  help: 'Total number of failed AI agent tool executions',
  labelNames: ['tool'],
  registers: [register],
});
